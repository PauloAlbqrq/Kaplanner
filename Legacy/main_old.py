...
def menu_vis():
    sel2 = int(
        input(
            'quais eventos deseja visualizar?\n1. do dia de hoje\n2. da semana\n3. do mês\n4. tudo\n> '
        ))
...
def agendar():
    ...
    tipo = int(
        input(
            'que tipo de compromisso deseja registrar?\n1.evento\n2.tarefa\n3.meta\n> '
        ))
    ...
    if tipo == 1:  #evento
        ...
        agendamento_agr = agendamento(get_date(), get_time(),
                                      planner_sess.usuario_planner,
                                      [agend_atual])
    ...
    if sel1 == 1:  #cadastro
        ...
        try:
            if users[cred_mail].get_senha() == cred_senha:
                os.system('clear')
                global planner_sess
                planner_sess = planner(usuario_planner=users[cred_mail],
                                       lista_agendamento=agendamentos)
                submenu1()
            else:
                os.system('clear')
